...
app = FastAPI(title="Membrane API")
...
app.add_middleware(
    ...
    allow_headers=["Content-Type", "X-Membrane-Client-Id"],
)
...
async def cleanup_with_openrouter(raw_chat: str, truncated: bool) -> str:
    ...
    headers = {
        ...
        "HTTP-Referer": "https://membrane.local",
        "X-Title": "Membrane",
    }
...
def root() -> dict[str, str]:
    return {"name": "Membrane API", "status": "ok", "health": "/health"}
...
async def capture_context(
    ...
    x_membrane_client_id: str | None = Header(default=None),
) -> dict[str, Any]:
    try:
        client_id = require_client_id(x_membrane_client_id)
